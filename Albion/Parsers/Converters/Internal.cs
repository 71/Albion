...
namespace Albion.Parsers
{
    ...
    {
        public static int Digit(string w)
        {
        ...
        /// <summary>
        /// Converts an integer to a literal string (up to 999 999).
        /// </summary>
        /// <param name="i"></param>
        /// <returns></returns>
        ...
        public static TimeSpan In(string r)
        {
            ...
            int hours = (match.Success) ? Int32.Parse(match.Value) : 0;
            match = Regex.Match(r, @"(\d+)(?= ?days?)", RegexOptions.IgnoreCase);
            int days = (match.Success) ? Int32.Parse(match.Value) : 0;
            match = Regex.Match(r, @"(\d+)(?= ?weeks?)", RegexOptions.IgnoreCase);
            int weeks = (match.Success) ? Int32.Parse(match.Value) : 0;
            days += weeks * 7;
